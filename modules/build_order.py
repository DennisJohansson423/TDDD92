...
class BuildOrder:
    ...
    def parse(file_name: str, separators: list[str]) -> list[Union[UNIT_TYPEID, UPGRADE_ID]]:
        ...
        if USE_DFBB:
            ...
            #rewrite file without our processed lines
            with open(file_name, "w") as f:
                for line in lines:
                    if line not in processed_lines:
                        f.write(line)
            #print("Order is: ", order) #here we can check our build order
        else:
            with open(file_name, "r") as f:
                for line in f:
                    # Extract what to build, ignore digits (supply/timestamp)
    ...
    def handle_edge_cases(name: str) -> str:
        ...
        :return: Name corresponding to correct unit/upgrade in commandcenter.
        """
