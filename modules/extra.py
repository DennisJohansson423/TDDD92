...
from commandcenter import UnitType, TypeData, PLAYER_SELF, Point2D, UPGRADE_ID, Point2DI, UNIT_TYPEID, Color, \
    BaseLocation, Race

from config import USE_DFBB ### Antti ###
from modules.unit_collection import *
...
def has_prerequisites(agent: BasicAgent, unit_type: Union[UnitType, UPGRADE_ID]) -> bool:
    ...
    ### Antti ###
    if USE_DFBB:
        if "MARINE" in str(unit_type.unit_typeid).upper():
            if len(agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_BARRACKS)) < 1:
                return False
            else:
                return True
        elif "TECHLAB" in str(unit_type.unit_typeid).upper():
            if len(agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_FACTORY)) < 1:
                return False
            else:
                return True
        elif "BAY" in str(unit_type.unit_typeid).upper():
            if len(agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_BARRACKS)) < 1:    
                return False
            else:
                return True
        elif "SCV" in str(unit_type.unit_typeid).upper():
            if len(agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SUPPLYDEPOT)) < 1:
                return False
            else:
                return True
