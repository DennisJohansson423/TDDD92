...
from tasks.potential_flow_scout import PotentialFlowScout
from commandcenter import PLAYER_SELF, PLAYER_ENEMY, UNIT_TYPEID, UnitType, UPGRADE_ID, Point2DI, Point2D
from modules.extra import exists_producer_for, get_worker_type, has_prerequisites
from queue import SimpleQueue
from config import DEBUG_CONSOLE, USE_POTENTIAL_FLOW

### Jonte ###
from config import USE_FLOOD_FILL
...
class TaskManager:
    ...
    def generate_tasks(self) -> None:
        ...
        # Send out scouting task every SCOUT FREQ frames.
        if USE_POTENTIAL_FLOW and self.agent.current_frame % self.SCOUT_FREQ == 1:
            self.scout()
        
        if not USE_POTENTIAL_FLOW and self.agent.current_frame == 1:
            self.scout()
    ...
    def get_production_task(self, type_to_build: Union[UnitType, UPGRADE_ID]) -> Optional[Task]:
        ...
        # type_to_build.unit_typeid == UNIT_TYPEID.TERRAN_BARRACKS
        if self.agent.primary_choke_point and USE_FLOOD_FILL and type_to_build.unit_typeid == UNIT_TYPEID.TERRAN_PLANETARYFORTRESS or USE_FLOOD_FILL and type_to_build.unit_typeid == UNIT_TYPEID.TERRAN_COMMANDCENTER: 
            inverted_y = self.agent.map_tools.height - self.agent.primary_choke_point[1] - 1
            choke_point = Point2DI(self.agent.primary_choke_point[0], inverted_y)
            pos = self.agent.py_building_placer.check_and_fix_building_place(choke_point, type_to_build)[1]
        else:
    ...
    def attack(self) -> None:
        ...
        if self.task_queue.is_empty() and self.agent.build_order.is_empty() and\
            ...
            for i in range(0, len(py_units)):
                self.task_queue.add(Attack(pos, self.ATTACK_PRIO, self.agent))
