...
from commandcenter import UnitType, Point2DI, Unit, PLAYER_SELF, UNIT_TYPEID
from tasks.task import Task, Status
from modules.py_unit import PyUnit
from modules import extra, unit_collection
from config import USE_DFBB ### Antti ###
...
class Build(Task):
    ...
    def on_start(self, py_unit: PyUnit) -> Status:
        ...
        ### Antti ###
        if USE_DFBB:
            stratometer = self.agent.get_stratometer()
            if stratometer == "attacking":
                if self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_SUPPLYDEPOT:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SUPPLYDEPOT)) > 9:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_BARRACKS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_BARRACKS)) > 5:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_FACTORY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_FACTORY)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_FACTORYTECHLAB:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_FACTORYTECHLAB)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_ARMORY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_ARMORY)) > 0:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_ENGINEERINGBAY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_ENGINEERINGBAY)) > 0:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_COMMANDCENTER:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_COMMANDCENTER)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_PLANETARYFORTRESS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_PLANETARYFORTRESS)) > 1:
                        return Status.FAIL_FAILED
            elif stratometer == "upgrading":
                if self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_ENGINEERINGBAY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_ENGINEERINGBAY)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_ARMORY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_ARMORY)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_REFINERY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_REFINERY)) > 2:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_BARRACKS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_BARRACKS)) > 1:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_SUPPLYDEPOT:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SUPPLYDEPOT)) > 5:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_COMMANDCENTER:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_COMMANDCENTER)) > 5:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_PLANETARYFORTRESS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_PLANETARYFORTRESS)) > 11:
                        return Status.FAIL_FAILED
            elif stratometer == "resources":
                if self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_ENGINEERINGBAY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_ENGINEERINGBAY)) > 0:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_REFINERY:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_REFINERY)) > 7:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_BARRACKS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_BARRACKS)) > 1:
                        return Status.FAIL_FAILED
                    ...
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_COMMANDCENTER)) > 3:
                        return Status.FAIL_FAILED
                elif self.building_type.unit_typeid == UNIT_TYPEID.TERRAN_PLANETARYFORTRESS:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_PLANETARYFORTRESS)) > 3:
                        return Status.FAIL_FAILED
