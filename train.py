...
from commandcenter import UnitType, UPGRADE_ID, ABILITY_ID, Point2D, Point2DI, UNIT_TYPEID, UnitType, UnitTypeID
    ...
    def on_start(self, py_unit: PyUnit) -> Status:
        ...
        if USE_DFBB:
            ...
            if stratometer == "attacking":
                if self.target.unit_typeid == UNIT_TYPEID.TERRAN_MARINE: #ändra tbx från target till unit_type?
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_MARINE)) > 49:
                        return Status.FAIL
                elif self.target.unit_typeid == UNIT_TYPEID.TERRAN_SIEGETANK:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SIEGETANK)) > 14:
                        return Status.FAIL
            elif stratometer == "upgrading":
                if self.target.unit_typeid == UNIT_TYPEID.TERRAN_SCV:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SCV)) > 29:
                        return Status.FAIL
                elif self.target.unit_typeid == UNIT_TYPEID.TERRAN_MARINE:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_MARINE)) > 9:
                        return Status.FAIL
            elif stratometer == "resources":
                if self.target.unit_typeid == UNIT_TYPEID.TERRAN_SCV:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_SCV)) > 49:
                        return Status.FAIL
                elif self.target.unit_typeid == UNIT_TYPEID.TERRAN_MARINE:
                    if len(self.agent.unit_collection.get_group(UNIT_TYPEID.TERRAN_MARINE)) > 9:
                        return Status.FAIL
